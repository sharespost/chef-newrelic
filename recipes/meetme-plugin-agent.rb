...
unless services.nil?
    require 'yaml'
    #hack: Ruby YAML adds !ruby/hash:Chef::Node::ImmutableMash, lets remove it...
    services_yml = services.to_yaml(:indentation => 2).gsub("! '#services':", '#services:').gsub('---','').gsub('!ruby/hash:Chef::Node::ImmutableMash','')
end
...
template "#{node['newrelic']['meetme_plugin_agent']['config_file']}" do
    ...
    mode "0644"
    variables(
        :license_key => node['newrelic']['meetme_plugin_agent']['license'],
